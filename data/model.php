...
if(isset($_GET['viewDeSn'])){  // ส่งตัวแปร GET ชื่อ viewUso มากำหนดให้ทำงานในส่วนนี้  
header("Content-type:application/json; charset=UTF-8");          
header("Cache-Control: no-store, no-cache, must-revalidate");         
header("Cache-Control: post-check=0, pre-check=0", false);     
$more_sql="";  
...
if(isset($_GET['addAp'])){ // ส่งตัวแปร GET ชื่อ addSch มากำหนดให้ทำงานในส่วนนี้  
    ...
    $de_pic=$mysqli->real_escape_string($data->de_pic);
    $de_locate=$mysqli->real_escape_string($data->de_locate);
...
if(isset($_GET['updateDeSn'])){ // ทำงานในส่วนนี้เมื่อมีค่าค่า get updateFriend และ Id เข้ามา  
    // อ่านค่าไฟล์ที่ถูกเขียนไว้ด้วย php แล้วนำค่าที่ได้มาเก็บในตัวแปร php object ชื่อ $data  
    $data = json_decode(file_get_contents("php://input"));  
   /* $smis=$mysqli->real_escape_string($data->smis); // เอาค่าจาก DB มาเก็บในตัวแปรอีกทีหนึ่ง  
    $name=$mysqli->real_escape_string($data->name); // เอาค่ามาเก็บในตัวแปรอีกทีหนึ่ง  
...
while($rs=$result->fetch_object()){  
    $json_data[]=array(  
        ...
        "ecr_compo_note"=>$rs->ecr_compo_note,
        "ecr_compo_solve"=>$rs->ecr_compo_solve,
...
if(isset($_GET['updateEcr'])){ // ทำงานในส่วนนี้เมื่อมีค่าค่า get updateFriend และ Id เข้ามา  
    ...
    $data = json_decode(file_get_contents("php://input"));  
        ...
        $ecr_tea_mail=$mysqli->real_escape_string($data->ecr_tea_mail);
        ...
        $ecr_compo_note=$mysqli->real_escape_string($data->ecr_compo_note);
        $ecr_compo_solve=$mysqli->real_escape_string($data->ecr_compo_solve);
    ...
    $data=array(  
        ...
        "ecr_compo_note"=>$ecr_compo_note,
        "ecr_compo_solve"=>$ecr_compo_solve,
...
$more_sql="";  
if(isset($_GET['Id']) && $_GET['Id']!=""){ // รับค่า กรณี แสดงรายการเดียวที่เลือก ตาม Id  
    $more_sql= "WHERE uso_smis='".$_GET['Id']."' ";  //$more_sql=" AND uso_smis='".$_GET['Id']."' ";  
}  
// เนื้อหาส่วนล่างถัดจากนี้ เป็นการเรียกใช้งาน คำสั่ง php ดึงข้อมูล  
// มาเก็บในตัวแปร array แล้วนำไปสร้างเป็นรูปแบบ json object   
// เนื้อหาก่อนหน้ามีรายละเอียด ในที่นี้ไม่ขออธิบายเพิ่ม  
...
////////////////////////////  JOB    ///////////////////////////
if(isset($_GET['viewJobSmis'])){  // ส่งตัวแปร GET ชื่อ viewUso มากำหนดให้ทำงานในส่วนนี้  
header("Content-type:application/json; charset=UTF-8");          
header("Cache-Control: no-store, no-cache, must-revalidate");         
header("Cache-Control: post-check=0, pre-check=0", false);     
$more_sql="";  
...
if(isset($_GET['viewJobId'])){  // ส่งตัวแปร GET ชื่อ viewUso มากำหนดให้ทำงานในส่วนนี้  
header("Content-type:application/json; charset=UTF-8");          
header("Cache-Control: no-store, no-cache, must-revalidate");         
header("Cache-Control: post-check=0, pre-check=0", false);     
$more_sql="";  
...
if(isset($_GET['addJob'])){ // ส่งตัวแปร GET ชื่อ addSch มากำหนดให้ทำงานในส่วนนี้  
    ...
    $data = json_decode(file_get_contents("php://input")); // ดึงข้อมูลจากไฟล์ มาใส่ตัวแปร object ชื่อ $data  
        ...
        $job_sch_name=$mysqli->real_escape_string($data->job_sch_name);
        $job_tea=$mysqli->real_escape_string($data->job_tea);
        $job_tea_tel=$mysqli->real_escape_string($data->job_tea_tel);
        $job_tea_mail=$mysqli->real_escape_string($data->job_tea_mail);
        $job_dea=$mysqli->real_escape_string($data->job_dea);
        $job_dea_tel=$mysqli->real_escape_string($data->job_dea_tel);
        $job_dea_mail=$mysqli->real_escape_string($data->job_dea_mail);
        $job_ser_id=$mysqli->real_escape_string($data->job_ser_id);
    ...
    $data=array(
        ...
        "job_type"=>$job_type, 
        "job_smis"=>$job_smis,
        "job_sch_name"=>$job_sch_name,
        "job_tea"=>$job_tea,
        "job_tea_tel"=>$job_tea_tel,
        "job_tea_mail"=>$job_tea_mail,
        "job_dea"=>$job_dea,
        "job_dea_tel"=>$job_dea_tel,
        "job_dea_mail"=>$job_dea_mail,
        "job_ser_id"=>$job_ser_id,
        "job_stat"=>$job_stat,
        "job_note"=>$job_note,
...
if(isset($_GET['delDe'])){  
    delete("device"," de_sn='".$_GET['Id']."' "); // ลบข้อมูลตาม de_sn
    exit;  
}  
...
if(isset($_GET['addImg'])){ // ส่งตัวแปร GET ชื่อ addSch มากำหนดให้ทำงานในส่วนนี้  
    // จะเห็นว่า การส่งค่าแบบ post มา จะไม่ได้รับค่าแบบตรงอ้างอิง $_POST['xxx'] ได้เลย  
    // แต่เป็นการไปอ่านค่าจากไฟล์ ที่ถูกเขียน ไว้ด้วย php อีกทีหนึ่ง  
    $data = json_decode(file_get_contents("php://input")); // ดึงข้อมูลจากไฟล์ มาใส่ตัวแปร object ชื่อ $data  
    
    
        $img_smis=$mysqli->real_escape_string($data->img_smis);  
        $img_de_sn=$mysqli->real_escape_string($data->img_de_sn);  
        $img_type=$mysqli->real_escape_string($data->img_type);
        $img_path=$mysqli->real_escape_string($data->img_path);
    ...
    insert("img",$data); // ฟังชั่นบันทึกข้อมูล ง่ายๆ ไม่กี่บรรทัด  
    exit;  
}  
...
if(isset($_GET['viewImgSn'])){  // ส่งตัวแปร GET ชื่อ viewUso มากำหนดให้ทำงานในส่วนนี้  
header("Content-type:application/json; charset=UTF-8");          
header("Cache-Control: no-store, no-cache, must-revalidate");         
header("Cache-Control: post-check=0, pre-check=0", false);     
$more_sql="";  
if(isset($_GET['Id']) && $_GET['Id']!=""){ // รับค่า กรณี แสดงรายการเดียวที่เลือก ตาม Id  
    $more_sql= " WHERE img_de_sn='".$_GET['Id']."' ";  //$more_sql=" AND uso_smis='".$_GET['Id']."' ";  
}  
// เนื้อหาส่วนล่างถัดจากนี้ เป็นการเรียกใช้งาน คำสั่ง php ดึงข้อมูล  
// มาเก็บในตัวแปร array แล้วนำไปสร้างเป็นรูปแบบ json object   
// เนื้อหาก่อนหน้ามีรายละเอียด ในที่นี้ไม่ขออธิบายเพิ่ม  
$sql="SELECT * FROM img $more_sql";  
$result = $mysqli->query($sql);  
while($rs=$result->fetch_object()){  
    $json_data[]=array(  
        //
        "img_id"=>$rs->img_id,      
        "img_date"=>$rs->img_date,  
        "img_smis"=>$rs->img_smis,  
        "img_de_sn"=>$rs->img_de_sn,   
        "img_type"=>$rs->img_type,
